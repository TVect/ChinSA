...
def squad_output(prefix, config, init_dict, network, input_tensor):
    ...
    pooler_kernel = init_dict["bert_pooler_dense_kernel"]
    pooler_bias = init_dict["bert_pooler_dense_bias"]
    # pooler = network.add_fully_connected(input_tensor, hidden_size, pooler_kernel, pooler_bias)
    pooler = network.add_fully_connected(
        slice_data, hidden_size, pooler_kernel, pooler_bias)
    pooler = network.add_activation(pooler.get_output(0), trt.ActivationType.TANH)

    W_out = init_dict[prefix + SQD_W]
    B_out = init_dict[prefix + SQD_B]
    W = network.add_constant((1, hidden_size, 1), W_out)
    dense = network.add_fully_connected(pooler.get_output(0), 1, W_out, B_out)
    # set_layer_name(dense, prefix, "dense")
    # set_output_name(dense, prefix, "logits")

    output = network.add_activation(dense.get_output(0), trt.ActivationType.SIGMOID)
    set_output_name(output, prefix, "output")
...
def emb_layernorm(builder, network, config, weights_dict, builder_config, sequence_lengths, batch_sizes):
    if len(batch_sizes) > 1 or len(sequence_lengths) > 1:
        ...
        input_shape = (-1 if len(batch_sizes) > 1 else batch_sizes[0], 
        ...
        shape = (batch_sizes[0], sequence_lengths[0])
        input_ids = network.add_input(name="input_ids", dtype=trt.int32, shape=shape)
        segment_ids = network.add_input(name="segment_ids", dtype=trt.int32, shape=shape)
        input_mask = network.add_input(name="input_mask", dtype=trt.int32, shape=shape)
...
def build_engine(batch_sizes, workspace_size, sequence_lengths, config, weights_dict, squad_json, vocab_file, calibrationCacheFile, calib_num):
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(explicit_batch_flag) as network, builder.create_builder_config() as builder_config:
        ...
        squad_logits = squad_output("", config, weights_dict, network, bert_out)
